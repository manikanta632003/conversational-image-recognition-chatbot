...
import './App.css';
// Removed Auth import since we're removing authentication

function App() {
  // Set a default user so the chat interface works without login
  ...
  const [factualMode, setFactualMode] = useState(false);
  ...
  useEffect(() => {
    // Removed authentication check since we're removing login
  ...
  useEffect(() => {
    // Removed user check since we're removing authentication
    scrollToBottom();
    loadSavedChats();
  ...
  }, [sessionId]); // Removed user dependency
  ...
  const resetChat = () => {
    if (window.confirm('Are you sure you want to reset everything? This will clear the chat, selected files, and input.')) {
  ...
  // Removed handleLogin and handleLogout functions since we're removing authentication

  // Removed the authentication check that was showing the Auth component
  // The chat interface will now be shown directly
        ...
        <div className="chat-header">
          <div className="header-top">
            ...
            <div className="header-actions">
              {/* Removed user info since there's no login */}
              <button onClick={() => {
              ...
              <button onClick={resetChat} className="icon-button" title="Reset Everything">
                🔄
              </button>
              {/* Removed logout button since there's no login */}
            </div>
          </div>
          ...
          <div className="language-controls">
            ...
            <div className="factual-mode-toggle">
              <label>
                <input
                  type="checkbox"
                  checked={factualMode}
                  onChange={(e) => setFactualMode(e.target.checked)}
                />
                📚 Factual Mode
              </label>
            </div>
